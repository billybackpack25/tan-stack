...
export async function createUpdateEvent(eventData) {
  let url = `http://localhost:3000/events`;

  if (eventData.id) url += `/${eventData.id}`;

  const response = await fetch(url, {
    method: eventData.id !== null ? 'PUT' : 'POST',
...
export async function fetchImages({ signal }) {
  ...
  if (!response.ok) {
    const error = new Error(
      'An error occurred while fetching the event images'
    );
    error.code = response.status;
    error.info = await response.json();
    throw error;
